...
const wrapCategories = function (categories) {
    return `<xml style="display: none">${
        categories.join('')
        }</xml>`;
};

const getGlobalExtensionBlocks = function () {
...
const getSpeechExtensionBlocks = function () {
    let xml = extensionsCategory;
    xml += speechGlobal;
    xml += speechLocal;
    if (hasWedo) {
...
const getWedoExtensionBlocks = function () {
    let xml = extensionsCategory;
    xml += wedoGlobal;
    xml += wedoLocal;
    if (hasSpeech) {
